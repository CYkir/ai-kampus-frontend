...
function stringToElements(str, opts, meta) {
	...
	for (let i = 0; i < tokens.length; i++) {
		...
		if (token.t === "nl" && opts.get("nl2br")) {
			...
			elements.push(import_react.createElement("br", { key }));
		} else if (!token.isLink || !opts.check(token)) elements.push(token.toString());
		else {
			let rendered = opts.render(token);
			if (!("key" in rendered.props)) {
				const key = `__linkify-el-${meta.elementId++}`;
				const props = Object.assign({ key }, rendered.props);
				rendered = import_react.cloneElement(rendered, props);
			}
...
function linkifyReactElement(element, opts, meta) {
	if (import_react.Children.count(element.props.children) === 0) return element;
	const children = [];
	import_react.Children.forEach(element.props.children, (child) => {
		if (typeof child === "string") children.push.apply(children, stringToElements(child, opts, meta));
		else if (import_react.isValidElement(child)) if (typeof child.type === "string" && opts.ignoreTags.indexOf(child.type.toUpperCase()) >= 0) children.push(child);
		else children.push(linkifyReactElement(child, opts, meta));
		else children.push(child);
	});
	const key = `__linkify-el-${meta.elementId++}`;
	const newProps = Object.assign({ key }, element.props);
	return import_react.cloneElement(element, newProps, children);
}
/**
* @template P
* @template {string | React.JSXElementConstructor<P>} T
...
var Linkify = (props) => {
	...
	const defaultLinkRender = ({ tagName, attributes, content }) => {
		...
		return import_react.createElement(tagName, attributes, content);
	};
	const newProps = { key: "__linkify-wrapper" };
	for (const prop in props) if (prop !== "options" && prop !== "as" && prop !== "tagName" && prop !== "children") newProps[prop] = props[prop];
	const opts = new Options(props.options, defaultLinkRender);
	const as = props.as || props.tagName || import_react.Fragment || "span";
	const children = props.children;
	return linkifyReactElement(import_react.createElement(as, newProps, children), opts, { elementId: 0 });
};
//#endregion
export { Linkify as default };

//# sourceMappingURL=linkify-react.js.map